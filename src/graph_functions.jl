...
function evaluateObjectiveFunction( inst::instance, b::bfsdata, SL::Array{Int,1}, SF::Array{Int,1}, wherefrom::String="OTHER_METHOD" )
    ...
    for w = 1:inst.Nsc
        ...
        if wherefrom == "OTHER_METHOD"
            if inst.mtype in [4,5]
                for i in 1:inst.nnodes
                    u  += b.yv[i] / (inst.leaderUtility * b.xv[i] + b.yv[i] + inst.r[i])                            #NOTE: yv is also used for IMP here, thus xv=0
                    uL += (inst.leaderUtility * b.xv[i]) / (inst.leaderUtility * b.xv[i] + b.yv[i] + inst.r[i])     #NOTE: yv is also used for IMP here, thus this is zero in the IMP
                    u_vals[w] += b.yv[i] / (b.xv[i] + b.yv[i] + inst.r[i])                                          #NOTE: yv is also used for IMP here
                end
            end
            if inst.mtype in [2,3]
                for i in 1:inst.nnodes
                    u  += b.yv[i]               #NOTE: yv is also used for IMP here, thus xv=0
                    uL += b.xv[i]               #NOTE: yv is also used for IMP here, thus this is zero in the IMP
                    u_vals[w] += b.yv[i]        #NOTE: yv is also used for IMP here
                end
            end
            if inst.mtype == 1
                for i in 1:inst.nnodes
                    u  += b.yf[i]               #NOTE: yv is also used for IMP here, thus xv=0
                    uL += b.xf[i]               #NOTE: yv is also used for IMP here, thus this is zero in the IMP
                    u_vals[w] += b.yf[i]        #NOTE: yv is also used for IMP here
                end
            end
        end
        if wherefrom == "BENDERS"
...
function evaluateObjectiveFunctionExPost( inst::instance, b::bfsdata, SL::Array{Int,1}, SF::Array{Int,1} )
    ...
    for _ in 1:inst.Nsc
        ...
        for i in 1:inst.nnodes
            ...
            uF_resistant  += b.yv[i] / (b.xv[i] + b.yv[i] + inst.r[i])                                                  #NOTE: yv is also used for IMP here, thus xv=0
            uL_resistant  += (inst.leaderUtility * b.xv[i]) / ((inst.leaderUtility * b.xv[i]) + b.yv[i] + inst.r[i])    #NOTE: yv is also used for IMP here, thus this is zero in the IMP
            uF_total      += b.yv[i]                                                                                    #NOTE: yv is also used for IMP here, thus xv=0
            uL_total      += b.xv[i]                                                                                    #NOTE: yv is also used for IMP here, thus this is zero in the IMP
            uF_forwards   += b.yf[i]                                                                                    #NOTE: yv is also used for IMP here, thus xv=0
            uL_forwards   += b.xf[i]
            #adjust viewings for organic case
            b.xv[i] > 1.0 ? b.xv[i] = 1.0 : nothing
            b.yv[i] > 1.0 ? b.yv[i] = 1.0 : nothing
            uF_organic      += b.yv[i]                                                                                  #NOTE: yv is also used for IMP here, thus xv=0
            uL_organic      += b.xv[i]                                                                                  #NOTE: yv is also used for IMP here, thus this is zero in the IMP
        end
    end
