...
# Author: Michael Kahr
# Runs with: 
#   - Ubuntu 22.04 / Windows 11 (without tracking memory consumption)
#   - Julia 1.10.0 
#   - CPLEX 22.1 (and solver-specific generic callbacks)
#******************************************************

# PARAMETERS TO BE SPECIFIED BY USER
if Sys.islinux()
    const CPLEX_DIR = "/opt/ibm/ILOG/CPLEX_Studio221/cplex/bin/x86-64_linux"  
end
if Sys.iswindows()
    const CPLEX_DIR = "C:\\Program Files\\CPLEX_Studio1210\\cplex\\bin\\x86-64_win\\" 
end
...
function main( params::Dict )
    ...
    for mySSAItr = 1:inst.nSSAItr
        ...
        #NOTE for callback context description see:
        #https://www.ibm.com/docs/en/icos/22.1.1?topic=manual-callback-context
        #NOTE: for enumerations to query in callbacks see:
        #https://www.ibm.com/docs/en/icos/22.1.1?topic=manual-cpxcallbackinfo

        cb_calls = Clong[]
        memorylimit::Int = deepcopy( params["memlimit"] )
        benf::Bool = deepcopy( params["benf"] )
        best_bnd_cur::Float64 = 0.0
        best_bnd_last::Float64  = Inf
        best_sol_cur::Float64 = 0.0
        best_sol_last::Float64  = 0.0
        mip_gap::Float64 = 0.0
        best_node::Int32 = 0
        function BendersOptimalityCuts( cb_data::CPLEX.CallbackContext, context_id::Clong )
            # lazy callback
            if context_id == CPX_CALLBACKCONTEXT_CANDIDATE
                ...
                if inst.kL == 0 #imp
                    if inst.mtype in [1,2,3,5] # F, T, O, R
                        @timeit to "LCUTS" startLazyBendersCallback( cb_data, context_id, inst, params, res, bfs, model )
                    else    #R with OA
                        @timeit to "LCUTS" startOACallbackIMP( cb_data, context_id, inst, params, res, bfs, model, "LCB" )
                    end
                else
                    if inst.mtype in [1,2,3,5] # F, T, O, R
                        @timeit to "LCUTS" startLazyBendersCallbackCIMP( cb_data, context_id, inst, params, res, bfs, model )
                    else    #R with OA
                        @timeit to "LCUTS" startOACallbackCIMP( cb_data, context_id, inst, params, res, bfs, model, "LCB" )
                    end
                end
            ...
            if context_id == CPX_CALLBACKCONTEXT_RELAXATION
                if benf == true
                    best_bnd_cur = cpx_callbackgetinfodbl( cb_data, CPXCALLBACKINFO_BEST_BND )
                    if (best_bnd_last - best_bnd_cur) > 0.001
                        if inst.kL == 0 #imp
                            if inst.mtype in [1,2,3,5] # F, T, O, R
                                @timeit to "UCUTS" startUserBendersCallback( cb_data, context_id, inst, params, res, bfs, model, to )
                            else    #R with OA
                                @timeit to "UCUTS" startOACallbackIMP( cb_data, context_id, inst, params, res, bfs, model, "UCB" )
                            end
                        else
                            if inst.mtype in [1,2,3,5] # F, T, O, R
                                @timeit to "UCUTS" startUserBendersCallbackCIMP( cb_data, context_id, inst, params, res, bfs, model )
                            else    # R with OA
                                @timeit to "UCUTS" startOACallbackCIMP( cb_data, context_id, inst, params, res, bfs, model, "UCB" )
                            end
                        end
                        best_bnd_last = best_bnd_cur
                    end
                end
                return
            end
            ...
            if context_id == CPX_CALLBACKCONTEXT_BRANCHING
                nodecount::Int32 = cpx_callbackgetinfoint( cb_data, CPLEX.CPXCALLBACKINFO_NODECOUNT )
                if (nodecount % 10) == 0
                    if Sys.islinux()
                        startProcStatus( cb_data, inst, memorylimit, res )
                    end
                end
 #               best_sol_cur    = cpx_callbackgetinfodbl( cb_data, CPLEX.CPXCALLBACKINFO_BEST_SOL )
 #               if best_sol_cur > best_sol_last
 #                   mip_gap = cpx_callbackget_mip_gap( cb_data )
 #                   best_sol_last = best_sol_cur
 #                   best_node = nodecount
 #                   println("node: $nodecount, \tbest_sol: $best_sol_cur, \t gap: $mip_gap")
 #               end
    ...
    #        println("context_id = ", context_id)
        ...
        # add callbacks
        if params["mtype"] in [1,2,3,4,5]  # all models
            MOI.set(model, CPLEX.CallbackFunction(), BendersOptimalityCuts)
        end
        ...
        if result_count( model ) > 0
            ...
            if (res.cpxStatus == "OPTIMAL") || (res.cpxStatus == "ALMOST_OPTIMAL") || (res.cpxStatus == "TIME_LIMIT") || (res.cpxStatus == "OTHER_ERROR")
                ...
                try
                    ...
                    res.nLCUTSC   = cpx_getnumcuts( model, CPLEX.CPX_CUT_TABLE )
                    res.nUCUTSC   = cpx_getnumcuts( model, CPLEX.CPX_CUT_USER )
    ...
    if( params["computeRankings"] )
        @timeit to "computeRankings" begin
            if !isempty(inst.SF)
                computeRankingsNew!( inst, res, inst.SF, "BEN", params, to )
            end
            if !isempty(inst.SFMAR)
                computeRankingsNew!( inst, res, inst.SFMAR, "MAR", params, to )
            end
        end
        Random.seed!( 666 );
...
#        println( "$arg  =>  $val" )
    ...
    if params["console"] == true
        #cd("/home/kahr/svn/projects/misc/logit_code_public/src")
