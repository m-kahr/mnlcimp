...
function computeSubgradient_all_models!( inst::instance, res::results, b::bfsdata, S::Vector{Tnodes}, w::Int, wherefrom::String )
    ...
    if inst.mtype == 1         #forwarding
        #compute β_i / γ_i NOTE: I use b.alpha here instead of beta, and γ is never used
        if wherefrom == "LCB"
            for i ∈ 1:inst.nnodes
                if ( b.xfb[i] == 0 )
                    b.alpha[i] = 1
...
#LAZY CUTS GBD
function startLazyBendersCallback( cb_data::CPLEX.CallbackContext, context_id::Clong, inst::instance, params::Dict, res::results, b::bfsdata, model::Model )
    #@info "in LCB"
    ...
    u_val = callback_value.(Ref(cb_data), model[:u]) 
...
function startUserBendersCallback( cb_data::CPLEX.CallbackContext, context_id::Clong, inst::instance, params::Dict, res::results, b::bfsdata, model::Model, to::TimerOutput )
    #@info "in UCB"
    eps::Float64 = 0.001
    relcuttol::Float64 = params["relcuttol"]
...
function startOACallbackIMP( cb_data::CPLEX.CallbackContext, context_id::Clong, inst::instance, params::Dict, res::results, b::bfsdata, model::Model, wherefrom::String )
    relcuttol::Float64 = params["relcuttol"]

    CPLEX.load_callback_variable_primal(cb_data, context_id)
    b.xv_val = callback_value.(Ref(cb_data), model[:xv]) 
    u_val = callback_value.(Ref(cb_data), model[:u]) 
